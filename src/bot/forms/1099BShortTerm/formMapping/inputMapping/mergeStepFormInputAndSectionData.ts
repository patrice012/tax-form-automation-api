...
export async function getMergeStepFormInputAndSectionData(data: any) {
  try {
    const inputMappings = await Promise.all([
      getEntryPointInputMapping(data),
      getGeneralInformationInputMapping(data),
      get4684InputMapping(data),
      getLessCommonScenariosInputMapping(data),
      getSaleOfHomeInputMapping(data),
      getSaleOfAssetInputMapping(data),
      getScheduleDInputMapping(data),
    ]);
    ...
    // logger.info(
    //   `Filtered Step Forms: ${JSON.stringify(filteredStepForms, null, 2)}`
    // );
    return filteredStepForms;
  } catch (error) {
